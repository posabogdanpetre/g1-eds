...
// Brand palette from the action payload — used to derive the card background.
...
// Compute a readable text color (dark on light, white on dark) for the secondary CTA.
...
const SECONDARY = PALETTE[1] || '#3de525';
...
function renderDetail(block, item, bridge) {
  ...
  cta.type = 'button';
  cta.textContent = 'Adauga in cos';
  cta.style.cssText = `background:${SECONDARY};color:${ctaTextColor(SECONDARY)};`;
  if (bridge) {
